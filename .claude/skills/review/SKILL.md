...
2. **Name the base.** The base ref is `origin/main` unless the user or workflow gave another one.
3. **Launch the reviewer.** Use the Agent tool with `subagent_type: "reviewer"`. The prompt is exactly these three parts, each under its heading, and nothing else:
   ...
   The reviewer collects the diff itself with `git diff`; do not paste it, and do not add your own opinion, the spec's notes, the run record, or a summary of what the implementer did.
